...
class TestTLSSpoofStrategy:
    ...
    async def test_fetch_blocks_ssrf_on_redirect(self):
        """fetch should block SSRF attempts when redirected to an unsafe URL."""
        mock_response_redirect = MagicMock()
        mock_response_redirect.text = "redirecting"
        mock_response_redirect.url = "https://example.com"
        ...
        with (
            patch("web_core.scraper.strategies.tls_spoof.is_safe_url", side_effect=[True, False]),
            pytest.raises(ValueError, match=r"SSRF blocked: http://169\.254\.169\.254/latest/meta-data/"),
        ):
            await strategy.fetch("https://example.com")

    async def test_fetch_follows_safe_redirects(self):
        """fetch should follow safe redirects up to 5 times."""
        mock_response_redirect = MagicMock()
        mock_response_redirect.text = "redirecting"
        mock_response_redirect.url = "https://example.com"
        mock_response_redirect.status_code = 301
        mock_response_redirect.headers = {"Location": "https://example.com/target"}

        mock_response_final = MagicMock()
        mock_response_final.text = "target"
        mock_response_final.url = "https://example.com/target"
