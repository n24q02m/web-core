...
class TestCaptchaStrategy:
    ...
    async def test_try_solve_turnstile_found(self):
        html = '<html><script>sitekey="1234567890abcdef"</script></html>'
        strategy = CaptchaStrategy(capsolver_api_key="key")
        with patch("web_core.scraper.strategies.captcha.detect_cloudflare_challenge", return_value="turnstile"), \
             patch("web_core.scraper.strategies.captcha.extract_turnstile_sitekey", return_value="123"), \
             patch.object(strategy, "solve_captcha", new_callable=AsyncMock, return_value="token123"):
            token = await strategy._try_solve_turnstile("https://example.com", html)
            assert token == "token123"

    async def test_try_solve_turnstile_not_found(self):
        strategy = CaptchaStrategy()
    ...
    async def test_try_solve_turnstile_no_sitekey(self):
        strategy = CaptchaStrategy()
        with patch("web_core.scraper.strategies.captcha.detect_cloudflare_challenge", return_value="turnstile"), \
             patch("web_core.scraper.strategies.captcha.extract_turnstile_sitekey", return_value=""):
            token = await strategy._try_solve_turnstile("https://example.com", "html")
            assert token == ""
    ...
    async def test_extract_turnstile_sitekey_data_attr(self):
        strategy = CaptchaStrategy()
        mock_page = AsyncMock()
        mock_el = AsyncMock()
        mock_el.get_attribute.return_value = "sitekey123"
        mock_page.query_selector.return_value = mock_el
        key = await strategy._extract_turnstile_sitekey(mock_page)
        assert key == "sitekey123"

    async def test_extract_turnstile_sitekey_iframe_src(self):
        strategy = CaptchaStrategy()
        mock_page = AsyncMock()
        mock_page.query_selector.return_value = None
        mock_iframe = AsyncMock()
        mock_iframe.get_attribute.return_value = "https://challenges.cloudflare.com/cdn-cgi/challenge-platform/h/g/turnstile/if/ov2/av0/rcv0/0/0x4AAAAAAADnPIDROrmt1Wwj/light/normal"
        mock_page.query_selector_all.side_effect = [[mock_iframe], []] # iframes, then scripts
        key = await strategy._extract_turnstile_sitekey(mock_page)
        assert key == "0x4AAAAAAADnPIDROrmt1Wwj"

    async def test_extract_turnstile_sitekey_script(self):
        strategy = CaptchaStrategy()
        mock_page = AsyncMock()
        mock_page.query_selector.return_value = None
        mock_script = AsyncMock()
        mock_script.text_content.return_value = "sitekey='0x4AAAAAAADnPIDROrmt1Wwj'"
        mock_page.query_selector_all.side_effect = [[], [mock_script]]
        key = await strategy._extract_turnstile_sitekey(mock_page)
        assert key == "0x4AAAAAAADnPIDROrmt1Wwj"
    ...
    async def test_solve_cf_turnstile_via_patchright_success(self):
        strategy = CaptchaStrategy(capsolver_api_key="key")
        ...
        with patch("web_core.browsers.patchright.PatchrightProvider", return_value=mock_provider), \
             patch.object(strategy, "_extract_turnstile_sitekey", return_value="sitekey123"), \
             patch.object(strategy, "solve_captcha", new_callable=AsyncMock, return_value="token123"):
    ...
    async def test_solve_cf_turnstile_via_patchright_no_sitekey(self):
        strategy = CaptchaStrategy(capsolver_api_key="key")
        mock_provider = AsyncMock()
        mock_browser = AsyncMock()
        mock_page = AsyncMock()
        mock_page.content.return_value = "<html>fail</html>"
        mock_provider.launch.return_value = mock_browser
        mock_browser.new_page.return_value = mock_page
    ...
    async def test_solve_cf_turnstile_via_patchright_no_token(self):
        strategy = CaptchaStrategy(capsolver_api_key="key")
        mock_provider = AsyncMock()
        mock_browser = AsyncMock()
        mock_page = AsyncMock()
        ...
        with patch("web_core.browsers.patchright.PatchrightProvider", return_value=mock_provider), \
             patch.object(strategy, "_extract_turnstile_sitekey", return_value="sitekey123"), \
             patch.object(strategy, "solve_captcha", new_callable=AsyncMock, return_value=""):
            result = await strategy._solve_cf_turnstile_via_patchright("https://example.com")
