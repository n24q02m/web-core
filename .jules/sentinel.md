...
## 2024-05-24 - Missing SSRF Validation in Adapter Image Download
**Vulnerability:** The `download_image` method in `MangaDexClient` constructed an image URL based on external data without explicitly calling `is_safe_url` to validate it against SSRF bypasses before performing the HTTP request.
**Learning:** Security controls built into factory methods like `safe_httpx_client` only protect the default execution path. When dependency injection is used (e.g., injecting an HTTP client instance in `MangaDexClient`), input parameters must be explicitly re-validated at the component level to maintain the security boundary and prevent SSRF bypasses. This is explicitly mandated by memory constraints.
**Prevention:** Always use `web_core.http.client.is_safe_url` to validate dynamically constructed URLs or user inputs in backend adapters before making HTTP requests, regardless of whether a safe factory HTTP client is used by default.
