...
def _install_searxng() -> bool:  # pragma: no cover
    ...
    try:
        ...
        if deps_result.returncode != 0:
            logger.error("Build deps installation failed", extra={"error": deps_result.stderr[:500]})
            return False
        ...
        logger.error("SearXNG installation failed", extra={"error": result.stderr[:500]})
        return False
    ...
    try:
        ...
        if _searxng_process.poll() is not None:
            if _searxng_process.stderr:
                ...
                stderr = ""
            logger.error("SearXNG process exited during startup", extra={"error": stderr[:500]})
        else:
            logger.warning(
...
async def _handle_restart_and_start(*, start_port: int) -> str:
    ...
    if _searxng_process is not None:
        ...
        logger.warning("SearXNG process crashed (exit_code=%s)", exit_code, extra={"error": stderr_output})
        _searxng_process = None
