...
async def _check_docker_daemon(docker_bin: str) -> bool:
    ...
    try:
        ...
        logger.error("Failed to check Docker daemon: %s", e)
        return False


async def _try_reuse_docker_container(docker_bin: str, container_name: str, url: str) -> bool:
    """Check if an existing container can be reused."""
    ...
    try:
        ...
        logger.error("Failed to reuse Docker container: %s", e)
        return False
...
async def _remove_docker_container(docker_bin: str, container_name: str) -> None:
    ...
    try:
        ...
        logger.warning("Failed to remove Docker container %s: %s", container_name, e)


async def _prepare_docker_settings(port: int) -> Path:
    """Prepare the settings.yml file for the Docker container."""
...
async def _start_docker_searxng(start_port: int) -> str | None:
    ...
    try:
        ...
        with lock.acquire(timeout=10):
            ...
            # Container not reusable, ensure clean state.
