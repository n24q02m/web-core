...
async def _kill_stale_port_windows(port: int) -> None:  # pragma: no cover
    """Windows-specific logic to kill a process on a given port."""
    try:
...
async def _kill_stale_port_unix(port: int) -> None:  # pragma: no cover
    """Unix-specific logic to kill a process on a given port."""
    try:
        result = await asyncio.to_thread(
            subprocess.run,
            ["lsof", "-ti", f":{port}"],
...
async def _kill_stale_port_process(port: int) -> None:  # pragma: no cover
    ...
    if sys.platform == "win32":
        await _kill_stale_port_windows(port)
    else:
