...
def _kill_stale_port_process_win32(port: int) -> None:  # pragma: no cover
    """Windows-specific logic to kill a process on a port using netstat."""
    try:
        result = subprocess.run(
            ["netstat", "-ano"],
            stdin=subprocess.DEVNULL,
...
def _kill_stale_port_process_unix(port: int) -> None:  # pragma: no cover
    """Unix-specific logic to kill a process on a port using lsof/fuser."""
    try:
        result = subprocess.run(
            ["lsof", "-ti", f":{port}"],
            stdin=subprocess.DEVNULL,
