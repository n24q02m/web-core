...
def _install_searxng_build_deps(pip_cmd: list[str]) -> bool:  # pragma: no cover
    ...
    try:
        ...
        logger.error("Build deps installation timed out")
        return False
    except Exception as e:
        logger.error("Failed to install build deps: %s", e)
        return False


def _validate_searxng_install_url() -> bool:  # pragma: no cover
...
def _run_searxng_package_install(pip_cmd: list[str]) -> bool:  # pragma: no cover
...
def _install_searxng() -> bool:  # pragma: no cover
    ...
    try:
        ...
        logger.error("Failed to coordinate SearXNG installation: %s", e)
        return False
