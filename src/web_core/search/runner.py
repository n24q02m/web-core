...
async def _prepare_searxng_port_and_env(start_port: int) -> tuple[int, dict[str, str]]:
    """Find port, kill stale processes, and prepare environment."""
...
def _build_searxng_command(port: int) -> list[str]:
    """Build the command to start SearXNG based on platform."""
    if sys.platform == "win32":
        return [
            sys.executable,
            "-c",
...
async def _verify_searxng_startup(url: str, port: int, proc: subprocess.Popen[bytes]) -> bool:
...
async def _handle_startup_failure(url: str, proc: subprocess.Popen[bytes]) -> None:
    """Log diagnostics and kill process on startup failure."""
...
async def _start_searxng_subprocess(start_port: int) -> str | None:  # pragma: no cover
    ...
    try:
        # 1. Prepare port and environment
        port, env = await _prepare_searxng_port_and_env(start_port)
        _searxng_port = port

        # 2. Build command and start process
        cmd = _build_searxng_command(port)
        ...
        # 4. Handle failure
        await _handle_startup_failure(url, _searxng_process)
        _searxng_process = None
        _searxng_port = None
        return None
