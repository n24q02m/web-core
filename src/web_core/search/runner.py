...
async def _prepare_docker_config(port: int) -> Path:
...
async def _run_docker_searxng(docker_bin: str, container_name: str, port: int, settings_path: Path) -> bool:
    """Execute docker run for SearXNG."""
    cmd = [
        docker_bin,
        "run",
        "--rm",
