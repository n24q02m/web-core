...
def _get_searxng_subprocess_cmd(port: int) -> list[str]:  # pragma: no cover
    """Get the command to start SearXNG subprocess."""
    if sys.platform == "win32":
        return [
            sys.executable,
            "-c",
...
async def _run_searxng_subprocess(
    cmd: list[str],
    env: dict[str, str],
    stderr_target: int | None,
) -> subprocess.Popen:  # pragma: no cover
    """Run the SearXNG subprocess."""
...
async def _handle_searxng_subprocess_failure(proc: subprocess.Popen, url: str) -> None:  # pragma: no cover
...
async def _start_searxng_subprocess(start_port: int) -> str | None:  # pragma: no cover
    ...
    try:
        ...
        await _handle_searxng_subprocess_failure(_searxng_process, url)
        _searxng_process = None
        _searxng_port = None
        return None
