...
async def _kill_stale_port_process_windows(port: int) -> None:  # pragma: no cover
    """Kill any process still holding the target port on Windows."""
    try:
...
async def _kill_stale_port_process_unix(port: int) -> None:  # pragma: no cover
    """Kill any process still holding the target port on Unix-like systems."""
    try:
        result = await asyncio.to_thread(
            subprocess.run,
            ["lsof", "-ti", f":{port}"],
...
async def _kill_stale_port_process(port: int) -> None:  # pragma: no cover
    ...
    if sys.platform == "win32":
        await _kill_stale_port_process_windows(port)
    else:
        await _kill_stale_port_process_unix(port)
