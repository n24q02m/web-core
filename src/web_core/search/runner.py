...
async def _try_reuse_docker_container(docker_bin: str, container_name: str, url: str, port: int) -> bool:
    """Try to reuse an existing Docker container if it is healthy.

    Returns True if reused successfully, updating global state.
    """
...
async def _cleanup_docker_container(docker_bin: str, container_name: str) -> None:
    """Forcefully remove a Docker container (ignores errors)."""
    await asyncio.to_thread(
        subprocess.run,
...
async def _prepare_docker_settings(port: int) -> Path:
    """Generate and write Docker-specific SearXNG settings.yml."""
...
async def _run_docker_container(
    ...
    """Execute docker run to start SearXNG."""
    cmd = [
        docker_bin,
        "run",
        "--rm",
...
async def _start_docker_searxng(start_port: int) -> str | None:
    ...
    try:
        ...
        with lock.acquire(timeout=10):
            ...
            await _cleanup_docker_container(docker_bin, container_name)
