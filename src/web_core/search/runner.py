...
async def _check_docker_daemon(docker_bin: str) -> bool:
    ...
    try:
        ...
        logger.warning("Failed to check Docker daemon: %s", e)
        return False


async def _try_reuse_docker_container(docker_bin: str, container_name: str, url: str) -> bool:
    """Check if a container is already running and healthy."""
    try:
        ...
        logger.warning("Failed to check existing Docker container: %s", e)
        return False
...
async def _prepare_docker_settings(port: int) -> Path:
    """Generate and write Docker-specific settings.yml."""
...
async def _start_docker_searxng(start_port: int) -> str | None:
    ...
    docker_bin = shutil.which("docker")
    if not docker_bin or not await _check_docker_daemon(docker_bin):
        return None
