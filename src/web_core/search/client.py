...
from web_core.http.url import extract_domain, is_valid_domain, normalize_url
...
def _get_shared_client(searxng_url: str) -> httpx.AsyncClient:
    ...
    hostname = urlparse(searxng_url).hostname or "localhost"
    client = _shared_clients.get(hostname)
    if client is None or getattr(client, "is_closed", False):
        client = safe_httpx_client(allow_private=[hostname], timeout=15.0)
        _shared_clients[hostname] = client
    return client
...
def _build_filtered_query(
    ...
    safe_include = _get_safe_domains(include_domains, _MAX_INCLUDE_DOMAINS)
    if safe_include:
        site_filter = " OR ".join(f"site:{d}" for d in safe_include)
        parts = [f"({site_filter}) {query}"]

    safe_exclude = _get_safe_domains(exclude_domains, _MAX_EXCLUDE_DOMAINS)
...
async def search(
    ...
    *,
    ...
    client = _get_shared_client(searxng_url)
