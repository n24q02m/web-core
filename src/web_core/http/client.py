...
def _check_ip_safe(ip_str: str, hostname: str) -> bool:
    ...
    Performance Optimization: Caching `_check_ip_safe` with `@functools.lru_cache`
    prevents redundant, expensive `ipaddress.ip_address` instantiations and
    property checks (`is_global`, `is_multicast`) during high-frequency SSRF
    validation within the DNS pinning cache.
