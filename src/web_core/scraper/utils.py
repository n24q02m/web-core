...
_STRIP_HTML_RE = re.compile(r"<(script|style)\b[^>]*>.*?</\1[^>]*>|<[^>]+>", re.IGNORECASE | re.DOTALL)
...
def visible_text(html: str) -> str:
    ...
    if not html:
        return ""
    # Performance Optimization: Combine script, style, and tag stripping into a single regex
    # pass to prevent intermediate string allocations and multiple regex engine runs.
    stripped = _STRIP_HTML_RE.sub(" ", html)
