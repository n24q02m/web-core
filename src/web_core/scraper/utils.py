...
# Cloudflare sitekey extraction patterns combined to prevent running the regex engine multiple times
_CF_SITEKEY_COMBINED = re.compile(
...
def extract_turnstile_sitekey(html: str) -> str | None:
    ...
    match = _CF_SITEKEY_COMBINED.search(html)
    if match:
