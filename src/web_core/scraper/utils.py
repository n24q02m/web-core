...
# Performance Optimization: Combining script block, style block, and generic HTML tag stripping
# into a single regex pass with alternations prevents creating intermediate strings and avoids
# running the regex engine multiple times, significantly speeding up execution on large HTML documents.
_STRIP_RE = re.compile(r"<(script|style)\b[^>]*>.*?</\1[^>]*>|<[^>]+>", re.IGNORECASE | re.DOTALL)
