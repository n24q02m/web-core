...
class CaptchaStrategy(BaseStrategy):
    ...
    async def _inject_turnstile_token(self, page: Any, token: str) -> bool:
        """Inject Turnstile token into the page and submit the challenge form."""
        try:
            await page.evaluate(
                """(token) => {
            ...
            logger.warning("Failed to inject Turnstile token: %s", e)
            return False
    ...
    async def _solve_cf_turnstile_via_patchright(self, url: str) -> ScrapingResult:
        ...
        try:
            ...
            try:
                ...
                if not await self._inject_turnstile_token(page, token):
                    content = await page.content()
                    return ScrapingResult(
                        content=content,
                        url=page.url,
                        strategy=self.name,
                        status_code=200,
                        metadata={"captcha_solved": False, "error": "token_injection_failed"},
                    )
