...
class PatchrightStrategy(BaseStrategy):
    ...
    def _get_provider(self) -> Any:
        """Get or instantiate the Patchright provider."""
    ...
    async def _navigate_and_detect(self, page: Any, url: str) -> tuple[Any, str, str | None]:
        """Navigate to URL and detect initial Cloudflare challenge."""
    ...
    async def _handle_managed_challenge(self, page: Any, url: str, content: str) -> tuple[str, str | None]:
        """Handle Cloudflare managed challenge."""
        logger.info("CF managed challenge for %s, polling for resolution", url)
        current_content = content
        cf_challenge_type: str | None = "managed"

        # Poll until challenge resolves (redirect to real page)
        for _ in range(_CF_POLL_MAX_CHECKS):
            await asyncio.sleep(_CF_POLL_INTERVAL)
            current_content = await page.content()
            cf_challenge_type = detect_cloudflare_challenge(current_content)
            if cf_challenge_type is None:
        ...
        return current_content, cf_challenge_type
    ...
    async def fetch(self, url: str, selectors: dict[str, str] | None = None) -> ScrapingResult:
        ...
        try:
            ...
            try:
                response, content, cf_challenge_type = await self._navigate_and_detect(page, url)
                ...
                if cf_challenge_type == "js_challenge":
                    ...
                    content, cf_challenge_type = await self._handle_managed_challenge(page, url, content)
